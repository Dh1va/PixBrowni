        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center">
          ...
          >
            ...
            <div className="flex flex-row gap-3 sm:gap-4 mb-10 md:mb-12">
              {/* PRIMARY: SOLID TO ORANGE */}
              <Link
                to="/contact"
                className="relative group overflow-hidden flex-1 sm:flex-none px-6 sm:px-10 py-4 sm:py-5 bg-slate-900 text-white rounded-[1.5rem] font-bold text-[10px] uppercase tracking-[0.3em] flex items-center justify-center gap-2 shadow-xl shadow-slate-900/10 active:scale-95"
              >
                <span className="relative z-10 flex items-center gap-2">
                  Free <span className="hidden sm:inline">Trial</span>
              ...
              {/* SECONDARY: OUTLINE TO ORANGE */}
              <Link
                to="/gallery"
                className="relative group overflow-hidden flex-1 sm:flex-none px-5 sm:px-10 py-3 sm:py-5 bg-transparent border-2 border-slate-200 text-slate-900 rounded-[1.5rem] font-bold text-[10px] uppercase tracking-[0.3em] flex items-center justify-center transition-all duration-300 active:scale-95"
              >
                <span className="relative z-10 transition-colors duration-300 group-hover:text-white">
                  Our Work
                </span>
                {/* Hover Animation matches the primary button but fills an empty space */}
                <div className="absolute inset-0 bg-orange-600 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]" />
              </Link>
            </div>
